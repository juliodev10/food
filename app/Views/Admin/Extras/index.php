            ...
            <div class="card-body">
                <div class="ui-widget">
                    <input id="query" name="query" class="form-control bg-light mb-4"
                        placeholder="Pesquise por uma extra de produto" />
                </div>

                <a href="<?= site_url("admin/extras/criar"); ?>" class="btn btn-success btn-sm
                    btn-icon-text float-right mb-4">
                    <i class="mdi mdi-plus btn-icon-prepend"></i> Cadastrar</a>

                <?php if (empty($extras)): ?>
                    <p>Nenhuma extra encontrada.</p>
                <?php else: ?>
                            ...
                            <tbody>
                                <?php foreach ($extras as $extra): ?>
                                    <tr>
                                        <td>
                                            <a
                                                href="<?= site_url('admin/extras/show/' . $extra->id); ?>"><?= $extra->nome; ?></a>
                                        </td>
                                        ...
                                        <td><?= ($extra->deletado_em === null ? '<label class="badge badge-success">Disponível</label>' : '<label class="badge badge-danger">Excluído</label>'); ?>
    ...
    jQuery(function($) {
        $("#query").autocomplete({
            ...
            select: function(event, ui) {
                ...
                window.location.href = "<?= site_url('admin/extras/show/') ?>" + ui.item.id;
            }
        });
    });
</script>
